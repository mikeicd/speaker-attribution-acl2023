...
class QuotationAttribution:
	...
	def tag(self, quotes, entities, tokens):
		...
		# quote_inds = ['TQ_'+str(i) for i in range(len(quotes))]
		...
		#add code to get quote text as well
		#add code to get style features
		#pass below to get batches of style features

		x_batches, m_batches, y_batches, o_batches, i_batches=self.model.get_batches(texts, metas, quote_indexes)
	...
	def get_representation(self, quotes, entities, tokens):
		...
		for q_id, (start_tok, end_tok) in enumerate(quotes):
			...
			for i in range(start, end):
				...
				if not in_quotes[i]:
					toks.append(tokens[i].text.lower())
					reverse_map.append(i)
				else:
					offset-=1
			...
			text=' '.join(toks)
			wp_toks=self.model.tokenizer.tokenize(text)
			if len(wp_toks) > 500:
