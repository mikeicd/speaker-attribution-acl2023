...
from booknlpen.english.bert_coref_quote_pronouns import BERTCorefTagger
import numpy as np
from booknlpen.common.pipelines import Entity
from booknlpen.english.name_coref import NameCoref
			...
			if ent.in_quote:
				for idx, (q_start, q_end) in enumerate(quotes):
					if ent.global_start >= q_start and ent.global_start <= q_end:
						ent.quote_mention=attributed_quotations[idx]            #probably to say that this mention cannot be the speaker? (unless its an I)
